...
static ngx_int_t
uri_app_stream(ngx_http_request_t *r, ngx_str_t *app, ngx_str_t *stream)
{
    ngx_uint_t  i = 0;
    ngx_str_t   pa[2];
    u_char     *start, *p, *last;
    ...
    for ( /* void */ ; p < last; p++) {
        ...
        if (*p == '/') {
            pa[i].data = ++p;
            if (i > 0) {
                pa[i - 1].len = (p - 1) - pa[i - 1].data;
            }
            i++;
        }
    }
    pa[i - 1].len = p - pa[i - 1].data;

    if (i < 2) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "flv live: uri app/stream invalid");
        return NGX_ERROR;
    }
    ...
    ngx_rtmp_connection_t *rconn)
{
    ...
    ngx_str_t                    port = ngx_string("1935");
    ngx_int_t                    in_port = 1935;
    ngx_uint_t                   i, n;
    ...
    if (uri_app_stream(r, &app, &stream) != NGX_OK) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "flv live: uri app/stream invalid");
        return NGX_ERROR;
    }
